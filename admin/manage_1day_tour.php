...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['update_experience'])) {
        try {
            $pdo->prepare("UPDATE site_sections SET title=?, subtitle=? WHERE section_key='1_day_exp_header'")->execute([$_POST['exp_header_title'], $_POST['exp_header_subtitle']]);
    ...
    if (isset($_POST['update_food_header'])) {
        try {
            $pdo->prepare("UPDATE site_sections SET title=?, subtitle=? WHERE section_key='1_day_food_header'")->execute([$_POST['food_title'], $_POST['food_subtitle']]);
            $message = "Header updated!"; $messageType = "success";
        } catch (Exception $e) { $message = "Error: " . $e->getMessage(); $messageType = "error"; }
    }
    if (isset($_POST['save_food'])) {
        try {
            ...
            if (!empty($_POST['food_id'])) {
                ...
                $pdo->prepare("INSERT INTO page_food_menu (page_key, title, items, image_url, is_highlighted, sort_order) VALUES ('1_day_tour', ?, ?, ?, ?, ?)")->execute([$_POST['title'], json_encode(array_values($items)), $image_url, $is_highlighted, $_POST['sort']]);
                $message = "Menu added!";
            }
            $messageType = "success";
        } catch (Exception $e) { $message = "Error: " . $e->getMessage(); $messageType = "error"; }
    ...
    if (isset($_POST['save_pricing'])) {
        try {
            ...
            if (!empty($_POST['price_id'])) {
                ...
                $pdo->prepare("INSERT INTO page_pricing (page_key, title, subtitle, price, price_unit, features, btn_text, style_mode, badge_text, sort_order) VALUES ('1_day_tour', ?, ?, ?, ?, ?, ?, ?, ?, ?)")
                    ->execute([$_POST['title'], $_POST['subtitle'], $_POST['price'], $_POST['unit'], json_encode(array_values($features)), $_POST['btn_text'], $_POST['mode'], $_POST['badge'], $_POST['sort']]);
                $message = "Plan added!";
            }
            $messageType = "success";
        } catch (Exception $e) { $message = "Error: " . $e->getMessage(); $messageType = "error"; }
...
$itinHeader = $pdo->query("SELECT * FROM site_sections WHERE section_key = '1_day_itin_header'")->fetch(PDO::FETCH_ASSOC);
...
<div class="max-w-6xl mx-auto pb-20">
    <div class="flex justify-between items-end mb-8">
        <div><h1 class="text-4xl font-bold text-safari-dark font-serif">1-Day Tour Manager</h1></div>
        <a href="../1-day-tour.php" target="_blank" class="text-safari-green font-bold hover:underline flex items-center gap-1">Preview <span class="material-symbols-outlined text-sm">visibility</span></a>
    </div>
