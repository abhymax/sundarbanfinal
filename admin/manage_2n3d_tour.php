    ...
    if (isset($_POST['update_hero'])) {
        try {
            ...
            if (!empty($_FILES['hero_image']['name'])) {
                ...
                $newFile = '2n3d_hero_'.time().'.'.$ext;
                if(move_uploaded_file($_FILES['hero_image']['tmp_name'], '../uploads/'.$newFile)) $image_url = 'uploads/'.$newFile;
            }
            // Smart Insert/Update
            $count = $pdo->query("SELECT COUNT(*) FROM site_sections WHERE section_key='{$PAGE_KEY}_hero'")->fetchColumn();
            if ($count > 0) {
                $pdo->prepare("UPDATE site_sections SET title=?, subtitle=?, cta_text=?, cta_link=?, image_url=?, overlay_opacity=? WHERE section_key='{$PAGE_KEY}_hero'")
                    ->execute([$_POST['hero_title'], $_POST['hero_subtitle'], $_POST['cta_text'], $_POST['cta_link'], $image_url, $_POST['overlay_opacity']]);
            } else {
                $pdo->prepare("INSERT INTO site_sections (section_key, title, subtitle, cta_text, cta_link, image_url, overlay_opacity) VALUES (?, ?, ?, ?, ?, ?, ?)")
                    ->execute(["{$PAGE_KEY}_hero", $_POST['hero_title'], $_POST['hero_subtitle'], $_POST['cta_text'], $_POST['cta_link'], $image_url, $_POST['overlay_opacity']]);
            }
    ...
    if (isset($_POST['update_experience'])) {
        try {
            ...
            foreach ($_POST['highlights'] as $id => $h) {
                ...
                if (!empty($_FILES['highlight_image_'.$id]['name'])) {
                    $ext = strtolower(pathinfo($_FILES['highlight_image_'.$id]['name'], PATHINFO_EXTENSION));
                    $newFile = '2n3d_hl_' . $id . '_' . time() . '.' . $ext;
                    if (move_uploaded_file($_FILES['highlight_image_'.$id]['tmp_name'], '../uploads/' . $newFile)) $image_url = 'uploads/' . $newFile;
                }
    ...
    if (isset($_POST['save_event'])) {
         try {
            ...
            if (!empty($_FILES['event_image']['name'])) {
                ...
                $newFile = '2n3d_ev_' . time() . '.' . $ext;
                if (move_uploaded_file($_FILES['event_image']['tmp_name'], '../uploads/' . $newFile)) $image_url = 'uploads/' . $newFile;
            }
            $day_num = (int)$_POST['day_number'];
            if (!empty($_POST['event_id'])) {
                $pdo->prepare("UPDATE page_timeline SET title=?, time_range=?, description=?, icon=?, color_theme=?, tags=?, image_url=?, sort_order=?, day_number=? WHERE id=?")->execute([$_POST['title'], $_POST['time'], $_POST['desc'], $_POST['icon'], $_POST['color'], $_POST['tags'], $image_url, $_POST['sort'], $day_num, $_POST['event_id']]);
                $message = "Event Updated!";
            } else {
                $pdo->prepare("INSERT INTO page_timeline (page_key, title, time_range, description, icon, color_theme, tags, image_url, sort_order, day_number) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")->execute([$PAGE_KEY, $_POST['title'], $_POST['time'], $_POST['desc'], $_POST['icon'], $_POST['color'], $_POST['tags'], $image_url, $_POST['sort'], $day_num]);
                $message = "Event Added!";
            }
        } catch (Exception $e) { $message = "Error: " . $e->getMessage(); }
    }
    ...
    if (isset($_POST['save_food'])) {
        try {
            ...
            if (!empty($_FILES['food_image']['name'])) {
                ...
                $newFile = '2n3d_food_' . time() . '.' . $ext;
                if (move_uploaded_file($_FILES['food_image']['tmp_name'], '../uploads/' . $newFile)) $image_url = 'uploads/' . $newFile;
            }
    ...
    if (isset($_POST['save_pricing'])) {
        try {
            ...
            if (!empty($_POST['price_id'])) {
                $pdo->prepare("UPDATE page_pricing SET title=?, subtitle=?, price=?, price_unit=?, features=?, btn_text=?, style_mode=?, badge_text=?, sort_order=? WHERE id=?")->execute([$_POST['title'], $_POST['subtitle'], $_POST['price'], $_POST['unit'], json_encode(array_values($features)), $_POST['btn_text'], $_POST['mode'], $_POST['badge'], $_POST['sort'], $_POST['price_id']]);
                $message = "Plan Updated!";
            } else {
                $pdo->prepare("INSERT INTO page_pricing (page_key, title, subtitle, price, price_unit, features, btn_text, style_mode, badge_text, sort_order) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")->execute([$PAGE_KEY, $_POST['title'], $_POST['subtitle'], $_POST['price'], $_POST['unit'], json_encode(array_values($features)), $_POST['btn_text'], $_POST['mode'], $_POST['badge'], $_POST['sort']]);
                $message = "Plan Added!";
            }
        } catch (Exception $e) { $message = "Error: " . $e->getMessage(); }
    }
...
// --- FETCH DATA (Safe Fallbacks) ---
$hero = $pdo->query("SELECT * FROM site_sections WHERE section_key = '{$PAGE_KEY}_hero'")->fetch(PDO::FETCH_ASSOC) ?: ['title'=>'', 'subtitle'=>'', 'image_url'=>'', 'overlay_opacity'=>'0.5', 'cta_text'=>'', 'cta_link'=>''];
...
$expHeader = $pdo->query("SELECT * FROM site_sections WHERE section_key = '{$PAGE_KEY}_exp_header'")->fetch(PDO::FETCH_ASSOC) ?: ['title'=>'', 'subtitle'=>''];
...
$itinHeader = $pdo->query("SELECT * FROM site_sections WHERE section_key = '{$PAGE_KEY}_itin_header'")->fetch(PDO::FETCH_ASSOC) ?: ['title'=>'', 'subtitle'=>''];
...
$foodHeader = $pdo->query("SELECT * FROM site_sections WHERE section_key = '{$PAGE_KEY}_food_header'")->fetch(PDO::FETCH_ASSOC) ?: ['title'=>'', 'subtitle'=>''];
$foodMenus = $pdo->query("SELECT * FROM page_food_menu WHERE page_key = '$PAGE_KEY' ORDER BY sort_order ASC")->fetchAll(PDO::FETCH_ASSOC) ?: [];
$priceHeader = $pdo->query("SELECT * FROM site_sections WHERE section_key = '{$PAGE_KEY}_price_header'")->fetch(PDO::FETCH_ASSOC) ?: ['title'=>'', 'subtitle'=>''];
...
<div class="max-w-6xl mx-auto pb-20">
    <div class="flex justify-between items-end mb-8">
        <div><h1 class="text-4xl font-bold text-safari-dark font-serif">2-Nights 3-Days Manager</h1></div>
        <a href="../2-nights-3-days-tour.php" target="_blank" class="text-safari-green font-bold hover:underline flex items-center gap-1">Preview <span class="material-symbols-outlined text-sm">visibility</span></a>
    </div>

    <?php if ($message || isset($_GET['msg'])): ?>
        <div class="p-4 mb-6 rounded-xl border bg-green-100 text-green-700 border-green-400 flex items-center gap-2">
            <span class="material-symbols-outlined">check_circle</span> <?php echo htmlspecialchars($message); ?>
        </div>
    <?php endif; ?>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-8">
            <form method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div><label class="block font-bold text-sm mb-2">Image</label><input type="file" name="hero_image" class="w-full text-sm"><input type="hidden" name="current_image_url" value="<?php echo htmlspecialchars($hero['image_url']); ?>"></div>
                    <div><label class="block font-bold text-sm mb-2">Opacity</label><input type="range" name="overlay_opacity" min="0" max="1" step="0.1" value="<?php echo htmlspecialchars($hero['overlay_opacity']); ?>" class="w-full"></div>
                </div>
    ...
    <div id="content-itinerary" class="tab-content hidden">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 mb-8">
            <form method="POST" class="flex gap-4 items-end">
                ...
                <div class="flex-1"><label class="block text-xs font-bold text-gray-500 mb-1">Subtitle</label><input type="text" name="itin_subtitle" value="<?php echo htmlspecialchars($itinHeader['subtitle']); ?>" class="w-full border rounded p-2 text-sm"></div>
                <div class="flex-1"><label class="block text-xs font-bold text-gray-500 mb-1">Title</label><input type="text" name="itin_title" value="<?php echo htmlspecialchars($itinHeader['title']); ?>" class="w-full border rounded p-2 font-bold text-lg font-serif"></div>
        ...
        <div class="flex justify-end mb-6"><button onclick="openEventModal()" class="bg-safari-green text-white px-6 py-2 rounded-xl font-bold flex items-center gap-2 shadow-lg hover:bg-green-800 transition"><span class="material-symbols-outlined">add</span> Add Time Slot</button></div>
        <div class="space-y-4">
    ...
    <div id="eventModal" class="fixed inset-0 z-50 hidden bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-y-auto max-h-[90vh]">
            <form method="POST" enctype="multipart/form-data" class="p-6">
                ...
                <div class="space-y-4">
                    <div><label class="block text-xs font-bold text-gray-600 mb-1">Select Day</label><select name="day_number" id="event_day" class="w-full border rounded p-2 font-bold bg-gray-50"><option value="1">Day 1</option><option value="2">Day 2</option><option value="3">Day 3</option></select></div>
                    <div class="grid grid-cols-2 gap-4"><div><label class="block text-xs font-bold text-gray-600 mb-1">Time</label><input type="text" name="time" id="event_time" placeholder="e.g. 5:00 AM" class="w-full border rounded p-2" required></div><div><label class="block text-xs font-bold text-gray-600 mb-1">Order</label><input type="number" name="sort" id="event_sort" value="10" class="w-full border rounded p-2"></div></div>
                    ...
                    <div class="grid grid-cols-2 gap-4"><div><label class="block text-xs font-bold text-gray-600 mb-1">Icon</label><input type="text" name="icon" id="event_icon" value="schedule" class="w-full border rounded p-2"></div><div><label class="block text-xs font-bold text-gray-600 mb-1">Color</label><select name="color" id="event_color" class="w-full border rounded p-2"><option value="orange">Orange</option><option value="green">Green</option><option value="blue">Blue</option><option value="yellow">Yellow</option><option value="gray">Gray</option></select></div></div>
                ...
                <div class="flex justify-end gap-3 mt-6 border-t pt-4"><button type="button" onclick="document.getElementById('eventModal').classList.add('hidden')" class="px-4 py-2 font-bold text-gray-500">Cancel</button><button type="submit" class="bg-safari-green text-white px-6 py-2 rounded-lg font-bold">Save Event</button></div>
            </form>
        </div>
    </div>
    ...
    <div id="foodModal" class="fixed inset-0 z-50 hidden bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-y-auto max-h-[90vh]">
            <form method="POST" enctype="multipart/form-data" class="p-6">
                ...
                <div class="space-y-4"><div><label class="block text-xs font-bold text-gray-600 mb-1">Title</label><input type="text" name="title" id="food_title" class="w-full border rounded p-2 font-bold" required></div><div><label class="block text-xs font-bold text-gray-600 mb-1">Items</label><textarea name="items" id="food_items" rows="5" class="w-full border rounded p-2 font-mono text-sm"></textarea></div><div class="grid grid-cols-2 gap-4"><div><label class="block text-xs font-bold text-gray-600 mb-1">Image</label><input type="file" name="food_image" class="w-full text-sm"><input type="hidden" name="current_food_image" id="current_food_image"></div><div><label class="block text-xs font-bold text-gray-600 mb-1">Order</label><input type="number" name="sort" id="food_sort" value="0" class="w-full border rounded p-2"></div></div><div><label><input type="checkbox" name="is_highlighted" id="food_highlight"> Highlighted</label></div></div>
                <div class="flex justify-end gap-3 mt-6 border-t pt-4"><button type="button" onclick="document.getElementById('foodModal').classList.add('hidden')" class="px-4 py-2 font-bold text-gray-500">Cancel</button><button type="submit" class="bg-safari-green text-white px-6 py-2 rounded-lg font-bold">Save</button></div>
            </form>
        </div>
    </div>
    ...
    <div id="priceModal" class="fixed inset-0 z-50 hidden bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-y-auto max-h-[90vh]">
            <form method="POST" class="p-6">
                ...
                <div class="space-y-4"><div class="grid grid-cols-2 gap-4"><div><label class="block text-xs font-bold text-gray-600 mb-1">Style</label><select name="mode" id="price_mode" class="w-full border rounded p-2"><option value="light">Light</option><option value="dark">Dark</option></select></div><div><label class="block text-xs font-bold text-gray-600 mb-1">Order</label><input type="number" name="sort" id="price_sort" value="0" class="w-full border rounded p-2"></div></div><div><label class="block text-xs font-bold text-gray-600 mb-1">Title</label><input type="text" name="title" id="price_title" class="w-full border rounded p-2 font-bold" required></div><div><label class="block text-xs font-bold text-gray-600 mb-1">Subtitle</label><input type="text" name="subtitle" id="price_subtitle" class="w-full border rounded p-2 text-sm"></div><div class="grid grid-cols-2 gap-4"><div><label class="block text-xs font-bold text-gray-600 mb-1">Price</label><input type="text" name="price" id="price_amt" class="w-full border rounded p-2 font-bold" required></div><div><label class="block text-xs font-bold text-gray-600 mb-1">Unit</label><input type="text" name="unit" id="price_unit" class="w-full border rounded p-2"></div></div><div><label class="block text-xs font-bold text-gray-600 mb-1">Features</label><textarea name="features" id="price_features" rows="4" class="w-full border rounded p-2 font-mono text-sm"></textarea></div><div class="grid grid-cols-2 gap-4"><div><label class="block text-xs font-bold text-gray-600 mb-1">Button Text</label><input type="text" name="btn_text" id="price_btn" class="w-full border rounded p-2"></div><div><label class="block text-xs font-bold text-gray-600 mb-1">Badge</label><input type="text" name="badge" id="price_badge" class="w-full border rounded p-2"></div></div></div>
                <div class="flex justify-end gap-3 mt-6 border-t pt-4"><button type="button" onclick="document.getElementById('priceModal').classList.add('hidden')" class="px-4 py-2 font-bold text-gray-500">Cancel</button><button type="submit" class="bg-safari-green text-white px-6 py-2 rounded-lg font-bold">Save</button></div>
            </form>
        </div>
    </div>
    ...
    <script>
        function switchTab(tab) {
            ...
            const btn = document.getElementById('tab-' + tab); btn.classList.remove('text-gray-500'); btn.classList.add('text-safari-green', 'border-b-4', 'border-safari-green');
        }
