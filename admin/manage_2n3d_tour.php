...
$PAGE_KEY = '2n3d_tour'; // Unique Key for 3-Day Tour
$message = '';
$messageType = '';
    ...
    if (isset($_POST['update_hero'])) {
        try {
            ...
            if (isset($_FILES['hero_image']) && $_FILES['hero_image']['error'] === UPLOAD_ERR_OK) {
                ...
                if (move_uploaded_file($_FILES['hero_image']['tmp_name'], '../uploads/2n3d_hero_' . time() . '.' . $ext)) {
                    $image_url = 'uploads/2n3d_hero_' . time() . '.' . $ext;
                }
            }
        ...
        try {
            ...
            foreach ($_POST['highlights'] as $id => $h) {
                ...
                if (isset($_FILES['highlight_image_'.$id]) && $_FILES['highlight_image_'.$id]['error'] === UPLOAD_ERR_OK) {
                    ...
                    if (move_uploaded_file($_FILES['highlight_image_'.$id]['tmp_name'], '../uploads/2n3d_hl_' . $id . '_' . time() . '.' . $ext)) {
                        $image_url = 'uploads/2n3d_hl_' . $id . '_' . time() . '.' . $ext;
                    }
                }
    ...
    if (isset($_POST['save_event'])) {
         try {
            ...
            if (isset($_FILES['event_image']) && $_FILES['event_image']['error'] === UPLOAD_ERR_OK) {
                ...
                if (move_uploaded_file($_FILES['event_image']['tmp_name'], '../uploads/2n3d_ev_' . time() . '.' . $ext)) {
                    $image_url = 'uploads/2n3d_ev_' . time() . '.' . $ext;
                }
            }
    ...
    if (isset($_POST['save_food'])) {
        try {
            ...
            if (isset($_FILES['food_image']) && $_FILES['food_image']['error'] === UPLOAD_ERR_OK) {
                ...
                if (move_uploaded_file($_FILES['food_image']['tmp_name'], '../uploads/2n3d_food_' . time() . '.' . $ext)) {
                    $image_url = 'uploads/2n3d_food_' . time() . '.' . $ext;
                }
            }
    ...
    <div id="content-itinerary" class="tab-content hidden">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 mb-8">
            <form method="POST" class="flex gap-4 items-end">
                ...
                <div class="flex-1"><label class="block text-xs font-bold text-gray-500 mb-1">Section Subtitle</label><input type="text" name="itin_subtitle" value="<?php echo htmlspecialchars($itinHeader['subtitle']); ?>" class="w-full border rounded p-2 text-sm"></div>
                <div class="flex-1"><label class="block text-xs font-bold text-gray-500 mb-1">Section Title</label><input type="text" name="itin_title" value="<?php echo htmlspecialchars($itinHeader['title']); ?>" class="w-full border rounded p-2 font-bold text-lg font-serif"></div>
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <select name="day_number" id="event_day" class="w-full border rounded p-2 font-bold bg-gray-50">
                            ...
                            <option value="3">Day 3</option> </select>
                    </div>
