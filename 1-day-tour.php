...
$pageKey = '1_day_tour'; // Database Key

// --- SAFE DATA FETCHING ---
try { $hero = $pdo->query("SELECT * FROM site_sections WHERE section_key = '1_day_hero'")->fetch(PDO::FETCH_ASSOC) ?: []; } catch(Exception $e) { $hero=[]; }
try { $cards = $pdo->query("SELECT * FROM page_cards WHERE page_key = '1_day_tour' AND section_key = 'quick_info' ORDER BY sort_order ASC")->fetchAll(PDO::FETCH_ASSOC) ?: []; } catch(Exception $e) { $cards=[]; }
try { $expHeader = $pdo->query("SELECT * FROM site_sections WHERE section_key = '1_day_exp_header'")->fetch(PDO::FETCH_ASSOC) ?: []; } catch(Exception $e) { $expHeader=[]; }
try { $highlights = $pdo->query("SELECT * FROM page_highlights WHERE page_key = '1_day_tour' AND section_key = 'experience' ORDER BY sort_order ASC")->fetchAll(PDO::FETCH_ASSOC) ?: []; } catch(Exception $e) { $highlights=[]; }
try { $itinHeader = $pdo->query("SELECT * FROM site_sections WHERE section_key = '1_day_itin_header'")->fetch(PDO::FETCH_ASSOC) ?: []; } catch(Exception $e) { $itinHeader=[]; }
try { $timeline = $pdo->query("SELECT * FROM page_timeline WHERE page_key = '1_day_tour' ORDER BY sort_order ASC")->fetchAll(PDO::FETCH_ASSOC) ?: []; } catch(Exception $e) { $timeline=[]; }
try { $foodHeader = $pdo->query("SELECT * FROM site_sections WHERE section_key = '1_day_food_header'")->fetch(PDO::FETCH_ASSOC) ?: []; } catch(Exception $e) { $foodHeader=[]; }
try { $foodMenus = $pdo->query("SELECT * FROM page_food_menu WHERE page_key = '1_day_tour' ORDER BY sort_order ASC")->fetchAll(PDO::FETCH_ASSOC) ?: []; } catch(Exception $e) { $foodMenus=[]; }
try { $priceHeader = $pdo->query("SELECT * FROM site_sections WHERE section_key = '1_day_price_header'")->fetch(PDO::FETCH_ASSOC) ?: []; } catch(Exception $e) { $priceHeader=[]; }
try { $pricingPlans = $pdo->query("SELECT * FROM page_pricing WHERE page_key = '1_day_tour' ORDER BY sort_order ASC")->fetchAll(PDO::FETCH_ASSOC) ?: []; } catch(Exception $e) { $pricingPlans=[]; }
...
<header class="relative h-[85vh] min-h-[600px] md:h-screen flex items-center justify-center overflow-hidden mb-8 md:mb-12">
    ...
    <div class="relative z-10 text-center px-4 max-w-4xl mx-auto mt-16 md:mt-0 w-full">
        <span
            class="inline-block py-1 px-3 rounded-full bg-orange-500/20 border border-orange-400 text-orange-100 text-xs md:text-sm font-bold tracking-widest uppercase mb-4 md:mb-6 backdrop-blur-sm animate-pulse">
        ...
        <h1 class="text-4xl md:text-7xl text-white font-bold mb-4 md:mb-6 hero-text leading-tight drop-shadow-lg">
            <?php echo htmlspecialchars($hero['title'] ?? 'Into the Wild'); ?>
        </h1>
        <p class="text-lg md:text-xl text-gray-100 mb-8 md:mb-10 max-w-2xl mx-auto font-light leading-relaxed drop-shadow-md">
            <?php echo htmlspecialchars($hero['subtitle'] ?? ''); ?>
        </p>
        
        <div class="flex flex-row gap-3 justify-center w-full max-w-md mx-auto">
            <a href="<?php echo htmlspecialchars($hero['cta_link'] ?? '#itinerary'); ?>"
                class="flex-1 bg-white text-green-900 px-2 py-3 md:py-4 rounded-full font-bold hover:bg-gray-100 transition transform hover:-translate-y-1 shadow-xl flex items-center justify-center gap-1 text-xs md:text-base whitespace-nowrap">
                <?php echo htmlspecialchars($hero['cta_text'] ?? 'View Itinerary'); ?> <span class="material-symbols-outlined text-sm md:text-lg">arrow_downward</span>
            </a>
            <a href="#map-section"
                class="flex-1 bg-transparent border-2 border-white text-white px-2 py-3 md:py-4 rounded-full font-bold hover:bg-white/10 transition transform hover:-translate-y-1 flex items-center justify-center gap-1 text-xs md:text-base whitespace-nowrap">
                Route Map <span class="material-symbols-outlined text-sm md:text-lg">map</span>
            </a>
        </div>
    </div>
                            ...
                            <div class="grid grid-cols-2 gap-3 md:gap-4 mt-4">
                                <?php foreach($bullets as $b): ?>
                                    <div class="bg-white p-3 md:p-4 rounded-lg shadow-sm border border-gray-100">
                                        ...
                                        <p class="font-bold text-xs md:text-sm"><?php echo htmlspecialchars($b); ?></p>
                                    </div>
                                <?php endforeach; ?>
                            </div>
                        <?php endif; ?>
...
<section id="itinerary" class="py-12 md:py-20 bg-white relative overflow-hidden">
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div class="relative">
            ...
            <?php if(!empty($timeline)): foreach ($timeline as $index => $event): 
                $isEven = ($index % 2 == 0);
            ...
            ?>
                <div class="bg-white rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition duration-300 group <?php echo $isHighlight ? 'border-t-4 border-orange-500 transform md:-translate-y-2' : ''; ?>">
                    <div class="h-40 md:h-48 overflow-hidden">
                        <img src="<?php echo $imgSrc; ?>" class="w-full h-full object-cover transform group-hover:scale-110 transition duration-500">
                    </div>
                    <div class="p-5 md:p-6">
                        <h3 class="text-lg md:text-xl font-bold text-gray-800 mb-2"><?php echo htmlspecialchars($menu['title']); ?></h3>
                        <ul class="text-gray-600 space-y-2 text-sm">
                            <?php foreach ($items as $item): ?>
                                <li class="border-b border-dashed pb-1 <?php echo $isHighlight && stripos($item, 'Fish')!==false ? 'font-bold text-orange-600' : ''; ?>">
    ...
    <div class="max-w-4xl mx-auto px-4 text-center">
        ...
        <div class="flex justify-center gap-8 flex-wrap">
            <div class="flex flex-col items-center">
                <div class="w-16 h-16 md:w-20 md:h-20 rounded-full border-4 border-orange-500 flex items-center justify-center text-lg md:text-xl font-bold text-orange-600 mb-2">
                    25%</div>
                <span class="text-xs md:text-sm font-bold">Royal Bengal Tiger</span>
            </div>
            <div class="flex flex-col items-center">
                <div class="w-16 h-16 md:w-20 md:h-20 rounded-full border-4 border-green-600 flex items-center justify-center text-lg md:text-xl font-bold text-green-700 mb-2">
                    90%</div>
                <span class="text-xs md:text-sm font-bold">Spotted Deer</span>
            </div>
            <div class="flex flex-col items-center">
                <div class="w-16 h-16 md:w-20 md:h-20 rounded-full border-4 border-blue-600 flex items-center justify-center text-lg md:text-xl font-bold text-blue-700 mb-2">
                    85%</div>
                <span class="text-xs md:text-sm font-bold">Estuarine Croc</span>
            </div>
            <div class="flex flex-col items-center">
                <div class="w-16 h-16 md:w-20 md:h-20 rounded-full border-4 border-yellow-500 flex items-center justify-center text-lg md:text-xl font-bold text-yellow-600 mb-2">
                    70%</div>
                <span class="text-xs md:text-sm font-bold">Monitor Lizard</span>
            </div>
        </div>
    </div>
</section>
