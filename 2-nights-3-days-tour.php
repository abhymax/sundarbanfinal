...
<header class="relative h-screen flex items-center justify-center overflow-hidden clip-path-wave mb-12">
    ...
    <div class="absolute bottom-20 left-1/2 transform -translate-x-1/2 animate-bounce text-white/80"><span class="material-symbols-outlined text-4xl">keyboard_double_arrow_down</span></div>
</header>
        ...
        <?php foreach ($highlights as $index => $h): $isEven = ($index % 2 == 0); $bullets = json_decode($h['list_data'], true); ?>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-12 items-center mb-20 last:mb-0">
                <div class="<?php echo $isEven ? 'order-2 md:order-1' : 'order-1 md:order-2'; ?> relative group">
                    ...
                    <img src="<?php echo htmlspecialchars($h['image_url']); ?>" class="relative rounded-2xl shadow-2xl w-full object-cover h-96 transform transition duration-500 hover:scale-[1.01]">
                    <?php if (!empty($h['badge_text'])): ?>
                        <div class="absolute bottom-4 right-4 bg-black/70 text-white px-3 py-1 rounded text-xs backdrop-blur-md"><?php echo htmlspecialchars($h['badge_text']); ?></div>
                    <?php endif; ?>
                </div>
                <div class="<?php echo $isEven ? 'order-1 md:order-2' : 'order-1'; ?> space-y-6">
                    <h3 class="text-2xl font-bold text-gray-800 flex items-center gap-3">
                        <span class="<?php echo $isEven ? 'bg-orange-100 text-orange-600' : 'bg-blue-100 text-blue-600'; ?> p-2 rounded-lg material-symbols-outlined"><?php echo $isEven ? 'campfire' : 'travel_explore'; ?></span>
...
<section id="itinerary" class="py-20 bg-white relative overflow-hidden">
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div class="flex justify-center mb-12 flex-wrap gap-2">
            <button onclick="switchDay(1)" id="tab-day1" class="px-6 md:px-8 py-3 rounded-full font-bold transition-all duration-300 bg-green-900 text-white shadow-md">Day 1: Arrival</button>
            <button onclick="switchDay(2)" id="tab-day2" class="px-6 md:px-8 py-3 rounded-full font-bold transition-all duration-300 text-gray-600 hover:bg-gray-100 shadow-sm">Day 2: Deep Jungle</button>
            <button onclick="switchDay(3)" id="tab-day3" class="px-6 md:px-8 py-3 rounded-full font-bold transition-all duration-300 text-gray-600 hover:bg-gray-100 shadow-sm">Day 3: Return</button>
        </div>
            ...
            ?>
                <div class="<?php echo $cardClass; ?> rounded-2xl p-8 shadow-2xl transform transition hover:scale-105 relative overflow-hidden">
                    <?php if (!empty($plan['badge_text'])): ?>
                        <div class="absolute top-0 right-0 bg-green-600 text-white px-4 py-1 rounded-bl-lg font-bold text-sm"><?php echo htmlspecialchars($plan['badge_text']); ?></div>
                    <?php endif; ?>
                    ...
                    <button onclick="openBooking()" class="w-full py-4 <?php echo $btnClass; ?> text-white rounded-xl font-bold transition shadow-lg"><?php echo htmlspecialchars($plan['btn_text']); ?></button>
                </div>
            <?php endforeach; ?>
        </div>
    </div>
...
<section class="pt-12 pb-56 bg-white">
    <div class="max-w-4xl mx-auto px-4 text-center">
        ...
        <div class="flex justify-center gap-8 flex-wrap">
            <div class="flex flex-col items-center"><div class="w-20 h-20 rounded-full border-4 border-orange-500 flex items-center justify-center text-xl font-bold text-orange-600 mb-2">25%</div><span class="text-sm font-bold">Royal Bengal Tiger</span></div>
            <div class="flex flex-col items-center"><div class="w-20 h-20 rounded-full border-4 border-green-600 flex items-center justify-center text-xl font-bold text-green-700 mb-2">90%</div><span class="text-sm font-bold">Spotted Deer</span></div>
            <div class="flex flex-col items-center"><div class="w-20 h-20 rounded-full border-4 border-blue-600 flex items-center justify-center text-xl font-bold text-blue-700 mb-2">85%</div><span class="text-sm font-bold">Estuarine Croc</span></div>
            <div class="flex flex-col items-center"><div class="w-20 h-20 rounded-full border-4 border-yellow-500 flex items-center justify-center text-xl font-bold text-yellow-600 mb-2">70%</div><span class="text-sm font-bold">Monitor Lizard</span></div>
        </div>
    </div>
</section>
